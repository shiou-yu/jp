...
import Product from './models/products.js' // ⚠️ 確保路徑對應到你的 Model

// 1. 資料庫連線字串 (請確認資料庫名稱是否正確)
const mongoURI = 'mongodb://127.0.0.1:27017/你的資料庫名稱' 
...
const importData = async () => {
  try {
    ...
    // 2. 清空舊有商品 (避免重複執行時資料堆疊)
    await Product.deleteMany({});
    console.log('舊商品已清空');

    // 3. 匯入新商品
    await Product.insertMany(products);
    console.log(`成功匯入 ${products.length} 項商品！`);

  } catch (error) {
    console.error('匯入出錯：', error.message);
  } finally {
    // 4. 關閉連線並退出
