  ...
  <v-app>
    <v-app-bar color="white" elevation="1" border="b">
      <v-container class="d-flex align-center">
        <v-app-bar-title class="font-weight-bold text-brown-darken-3" style="letter-spacing: 1px;">
          日本超商商品評論網
        </v-app-bar-title>
        ...
        <template v-for="nav in navs" :key="nav.to">
          <v-btn
            v-if="nav.show"
            class="text-brown-darken-3 font-weight-bold"
            :to="nav.to"
            variant="text"
          >
    ...
    <v-footer class="py-4" color="white" border="t">
...
const logout = async () => {
  ...
  createSnackbar({
    text: '登出成功',
    snackbarProps: {
      color: 'brown-darken-2', // 提示框也改為深棕色，更有氣質
    },
  })
}
</script>
