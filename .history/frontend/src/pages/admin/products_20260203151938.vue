          ...
          >
            ...
            <template #[`item.sell`]="{ value }">
              <v-chip
                ...
                variant="flat"
              >
                <v-icon :icon="value ? 'mdi-check' : 'mdi-close'" size="small" start />
                {{ value ? '上架中' : '未上架' }}
              </v-chip>
            </template>
    ...
    <v-form :disabled="form.isSubmitting.value" @submit.prevent="submit">
      <v-card class="dialog-card">
        ...
        <div class="dialog-header">
          <v-icon color="white" size="48">{{ dialog.id ? 'mdi-pencil' : 'mdi-plus-circle' }}</v-icon>
        ...
        <v-card-actions class="pa-4 bg-grey-lighten-4">
          ...
          <v-btn
            ...
            variant="flat"
          >
            <v-icon class="mr-2" left>{{ dialog.id ? 'mdi-content-save' : 'mdi-check' }}</v-icon>
            {{ dialog.id ? '儲存' : '確認' }}
          </v-btn>
        </v-card-actions>
      </v-card>
