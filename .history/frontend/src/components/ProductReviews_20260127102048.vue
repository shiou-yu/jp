  ...
  <v-container class="py-0">
    <v-row class="mt-10" justify="center">
      <v-col cols="12" lg="4" md="6">
        <v-card class="rounded-xl overflow-hidden border-thin" variant="flat">
          <v-img
            v-if="product?.image"
            alt="商品圖片"
            aspect-ratio="1"
          ...
          <v-sheet
            ...
            height="300"
          >
            <v-icon color="grey-lighten-2" size="64">mdi-image-outline</v-icon>
          </v-sheet>
        </v-card>

        <div v-if="product" class="text-center mt-4">
          <h1 class="text-h5 font-weight-bold text-blue-grey-darken-4">{{ product.name }}</h1>
        </div>
      </v-col>
    </v-row>
...
const fetchProduct = async () => {
  try {
    ...
    createSnackbar({ text: '讀取商品資訊失敗', snackbarProps: { color: 'red' } })
  }
}
