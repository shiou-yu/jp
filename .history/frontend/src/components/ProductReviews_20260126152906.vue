<template>
  ...
  >
    ...
    <v-card-title class="font-weight-bold text-blue-grey-darken-4 pb-0">
      {{ name }}
    </v-card-title>
    ...
    <div class="px-4 py-1 d-flex align-center">
      <v-rating
        ...
        size="x-small"
      ></v-rating>
      <span class="text-caption text-blue-grey-lighten-2 ml-1">
        ({{ reviewCount }} 則評論)
      </span>
    </div>
    ...
    <v-card-subtitle class="pt-2 d-flex align-center justify-space-between">
      ...
      <span class="font-weight-bold text-h6 text-blue-grey-darken-3">{{ formatedPrice }}</span>
    </v-card-subtitle>

    <v-card-text class="text-blue-grey-darken-1 flex-grow-1 mt-2">
      {{ description }}
    </v-card-text>
